...
class UserRepository {
    ...
    fun getSearchableSpeciality(text: String, response: MutableLiveData<Resource<List<ItemSearch>>>) {
        compositeDisposable.add(
            apiClient.getSpeciality("Bearer ${Constants.token}")
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeWith(object : DisposableObserver<List<SpecialityItemResponse>>() {
                    override fun onNext(t: List<SpecialityItemResponse>) {
                        val list = t.filter {
                            ...
                            ItemSearch(it.id!!,it.name ?: "" )

                        }
                        response.value = Resource(Status.SUCCESS, list, null, null)
                    }
    ...
    fun getSearchableProfileSettings(
        text: String,
        response: MutableLiveData<Resource<List<ItemSearch>>>,
    ) {
        val list = ArrayList<ItemSearch>()
        list.add(ItemSearch(0, "Изменить номер телефона"))
        list.add(ItemSearch(1, "Изменить пароль"))
        list.add(ItemSearch(2, "Личные данные"))
        list.add(ItemSearch(3, "Мои документы"))
        list.add(ItemSearch(4, "Мои врачи"))
        list.add(ItemSearch(5, "Языки"))
    ...
    fun getSearchableMains(text: String, response: MutableLiveData<Resource<List<ItemSearch>>>) {
        val list = ArrayList<ItemSearch>()
        list.add(ItemSearch(0, "Главная"))
        list.add(ItemSearch(1, "Наши врачи"))
        list.add(ItemSearch(2, "Эстетическая медицина"))
        list.add(ItemSearch(3, "CПА-комплекс"))
        list.add(ItemSearch(4, "Адрес и контакты"))
        list.add(ItemSearch(5, "Личный кабинет"))
    ...
    fun getFilteredDoctors(
        ...
        compositeDisposable.add(
            apiClient.getFilteredDoctors(date,
                specialityId,
                subSpecialityId,
    ...
    fun getMonthlyDate(
        doctorId: Int,
        response: MutableLiveData<Resource<List<MonthlyDateResponse>>>,
    ) {
        compositeDisposable.add(
        ...
        compositeDisposable.add(
            apiClient.getMyDoctors("user@user.com", "Bearer ${Constants.token}")
                ...
                .subscribeWith(object : DisposableObserver<List<DoctorResponse>>() {
                    override fun onNext(t: List<DoctorResponse>) {
                        val list = t.filter {
                            it.fullName?.contains(text, false) ?: false
                        }.map {
                            it
                        }
                        response.value = Resource(Status.SUCCESS, list, null, null)
    ...
    fun getChatMessages(
        response: MutableLiveData<Resource<List<ChatMessagesResponse>>>,
        chatId: Int,
    ) {
        compositeDisposable.add(
